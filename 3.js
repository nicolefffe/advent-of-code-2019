...
const getMoves = (move, coordinates) => {
    ...
    for (let i = 1; i <= distance; i++) {
        switch (direction) {
            case 'U':
                [x, y] = goUp(i, coordinates);
                break;
            case 'D':
                [x, y] = goDown(i, coordinates);
                break;
            case 'R':
                [x, y] = goRight(i, coordinates);
                break;
            case 'L':
...
const findCoordinates = (valuesToSearch, coordToFind) => {
    let start = 1;
    let end = valuesToSearch.length - 1;
    let mid, coordToTry, compare;
...
const findIntersection = (path1, path2) => {
    ...
    if (path1.length > path2.length) {
        shortPath = sortPath(path2);
        longPath = sortPath(path1);
    } else {
        shortPath = sortPath(path1);
        longPath = sortPath(path2);
    }

    let foundCoord;
    for (let i = 1; i < longPath.length - 1; i++) {
